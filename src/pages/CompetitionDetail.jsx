...
import { useUser } from "../contexts/UserContext"; // (로그인 기능이 있다면)
...
export default function CompetitionDetail() {
  const { id } = useParams();              // URL 파라미터(:id)를 받아온다.
  const { user } = useUser();             // (로그인 기능이 있다면)

  const [competition, setCompetition] = useState(null);
  const [isLoading, setIsLoading] = useState(true);
  const [showTeamModal, setShowTeamModal] = useState(false);
  const [applicantType, setApplicantType] = useState("");
  const [users, setUsers] = useState([]);                       // (팀매칭용 샘플 유저 리스트)
  ...
  // 2) 팀매칭용 “사용자 목록”을 mock으로 세팅 (실제라면 별도의 API가 필요합니다)
  useEffect(() => {
    const mockUsers = [
  ...
  const handleTeamSubmit = async () => {
    ...
    if (applicantType === "개인") {
      ...
      try {
        await API.post("/api/team-match/apply", payload);
        alert("✅ 신청 완료!");
        closeModal();
      } catch (e) {
        console.error(e);
      ...
      const incomplete = selectedTeamMembers.some((uid) => !memberInterests[uid]);
      if (incomplete) {
        alert("모든 팀원의 관심 분야를 선택해주세요.");
        return;
      }
      const members = selectedTeamMembers.map((uid) => ({
        id: uid,
        interest: memberInterests[uid],
      }));
      const payload = {
        competitionId: competition.id,
        applicantType,
  ...
  // 사용자별 “관심 분야” 버튼을 렌더링하는 헬퍼 함수
  const renderInterestButtons = (userId) => {
    const selectedMain = mainCategories[userId]; // 현재 유저가 선택한 대분류
    return (
            ...
            <div className="grid grid-cols-2 gap-2">
              {categoryDetails[selectedMain].map((item) => (
                <button
                  key={item}
                  onClick={() =>
                    setMemberInterests({ ...memberInterests, [userId]: item })
                  }
                  className={`border px-3 py-1 rounded text-sm ${
                    memberInterests[userId] === item
  ...
  if (!competition) {
    return <div className="p-6">공모전 정보를 찾을 수 없습니다.</div>;
  }

  return (
    <div className="p-8">
      <h1 className="text-2xl font-bold mb-2 text-sky-700">{competition.title}</h1>
      <p className="text-gray-600 mb-2">
        {competition.category} / {competition.subcategory}
      </p>
      {competition.host && (
        <p className="text-sm text-gray-500 mb-1">주최: {competition.host}</p>
      )}
      {competition.deadline && (
        <p className="text-sm text-gray-500 mb-4">마감일: {competition.deadline}</p>
      )}
      {competition.link && (
        <p className="mb-4">
          <a
                ...
                <div className="space-y-4">
                  ...
                  <div className="border rounded max-h-32 overflow-y-auto p-2 space-y-1">
                    {users
                      ...
                      .map((u) => {
                        const isDisabled =
                          teamSize && selectedTeamMembers.length >= parseInt(teamSize);
                    ...
                    <div key={uid} className="bg-gray-50 border rounded p-2">
                      <div className="flex justify-between items-center mb-2">
                        ...
                        <button
                          onClick={() => {
                            setSelectedTeamMembers(
                              selectedTeamMembers.filter((id) => id !== uid)
                            );
